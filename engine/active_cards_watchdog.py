...
class CardSTSScheduler:
    ...
    def can_poll(self, now: float, current_score: str, stage_rank: int) -> bool:
        # Zablokuj zapytania w stanach końcowych: FT (40)
        if stage_rank >= 40:
            return False
...
class ActiveCardsWatchdog:
    ...
    def check_active_cards_once(self, live_matches: Optional[List[Dict[str, Any]]] = None, sts_matches: Optional[List[Dict[str, Any]]] = None) -> List[Dict[str, Any]]:
        ...
        for card_key, card in cards_snapshot.items():
            ...
            if sts_url and '/live/' in sts_url and scheduler.can_poll(now, current_score, inc_rank):
                ...
                try:
                    ...
                    if real_markets:
                        ...
                        card["cached_real_markets"] = real_markets
            ...
            # Gwarancja zachowania autentycznych kursów STS (brak degradacji do syntetycznych)
            if card.get("cached_real_markets") and (not best_match.get('live_markets') or not any(m.get('source') == 'STS_REAL' for m in best_match.get('live_markets', []))):
                best_match['live_markets'] = card["cached_real_markets"]
